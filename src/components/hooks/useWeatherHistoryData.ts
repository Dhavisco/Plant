...
// src/hooks/useWeatherHistoryData.ts
...
const fetchWeatherHistoryData = async (location: string, startDate: string, endDate: string) => {
  const response = await axiosInstance.get(
    `/weather/history/${location}/${startDate}/${endDate}`
  );
  ...
  return dailyData.time.map((date: string, index: number) => ({
    time: date,
    temperature_2m_max: dailyData.temperature_2m_max[index],
    temperature_2m_mean: dailyData.temperature_2m_mean[index],
    rain_sum: dailyData.rain_sum[index],
...
const useWeatherHistoryData = (location: string, startDate: string, endDate: string) => {
  return useQuery({
    queryKey: ["weatherHistory", location, startDate, endDate], 
    queryFn: () =>fetchWeatherHistoryData(location, startDate, endDate)
  });
};
